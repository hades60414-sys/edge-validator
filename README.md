...
裁判引擎(`engine/judge_web.py`,約 1,800 行)是純 Python + numpy/pandas,透過 [Pyodide](https://pyodide.org) 編譯成 WASM 在瀏覽器內跑。同一份引擎程式碼在 CPython 下直接 `pytest`(137 個測試,其中數支專門對帳本 README 的數字與校準宣稱、漂移即紅),前端只是把它注入 Pyodide——**測試環境與生產環境是同一份程式碼**,不存在「JS 重寫一遍然後兩邊算不一樣」的問題。
...
校準揭露(誠實聲明):SPA / Romano–Wolf 這兩道 FWER 閘經蒙地卡羅校準與獨立複驗——iid 情境(T=250/500)下,家族偽陽率落在名目水準內(多輪獨立複測一致);但**中度以上自相關資料(如 AR(1)=0.30)下,block bootstrap 的有限樣本殘餘偽陽會比名目高約 +1~3pp**(名目 10%/5% 實測約 12.5%/7.5%,與引擎 docstring 及獨立複測一致、在蒙地卡羅誤差範圍內)。這兩道閘的誠實定位是「已校準(誤差 ±3pp 內)」、不宣稱精確——讀 SPA/RW 結果時請把這一點放在心上。
